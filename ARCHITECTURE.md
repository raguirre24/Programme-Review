...
- **Query Expressions (`expressions.tmdl`)**:
  ...
  - `fnXerCsvReadBundleTable`: For each selected project, sums manifest `row_count` across all source snapshots for the requested table, streams the corresponding CSV, validates its data-row count after its header, then applies types and schema `1.0`/`2.0` compact-key conversion. A mismatch raises contextual `XerCsvRowCount`; large CSV binaries and tables are not buffered.
  - `XER CSV Refresh Audit`: Hidden calculated table that returns `PASS` or `DIAGNOSTIC_MISMATCH` for each of the ten row-count records. It still fails refresh through `ERROR()` for Athena/CSV ownership overlap or a routed project missing from `dbo_project` or `dbo_userpermission`. Manifest content never grants RLS access.
  - **Parameters**:
...
- **Ownership rule**: CSV routing is project-level. When active, Athena excludes both recognised C/J aliases of each currently routed project inside the native SQL scope; all ten imported XER tables then append the selected CSV bundle data. A project cannot be owned by both sources. With `SelectedProjects = ALL`, `XerCsvProjectCodes` changes only current source ownership: `C6036` routes C6036 to CSV and every other project to Athena; switching it to `CNZ01` routes CNZ01 to CSV and returns C6036 to Athena when records exist; selecting `C6036,CNZ01` routes both to CSV and every other project to Athena. A former CSV project with no Athena records contributes zero rows without a routing error. With a finite selection, every CSV-routed project must remain inside `SelectedProjects`. When that finite list is entirely CSV-owned, all ten XER Athena branches return typed empty tables without opening `AthenaDB`; governed RLS tables remain Athena-authoritative. Tables `04 XER_BASELINE` and `06a XER_SUCCESSOR` remain calculated model tables.
...
4. Reopen Power BI Desktop and run a full refresh. Verify `IsCsvSource`, compact keys, all ten audit records, Athena-only, CSV-only and mixed ownership, baseline/previous-period logic, network links, codes, calendars, resources and RLS **View as**.
...
- Functional acceptance requires identical business row counts and values, no increase in VertiPaq rows, no audit error, `PASS` on all ten diagnostic records and one table-data read per CSV table/bundle. Zero heavy XER Athena queries applies only when every project in a finite `SelectedProjects` list is CSV-owned; `SelectedProjects = ALL` deliberately retains the broad Athena workload for every non-routed project.
